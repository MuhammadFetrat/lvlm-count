...
WORKING_DIRECTORY_BASE_NAME = f"tmp_pascal_{datetime.now().strftime('%d.%m.%Y-%H:%M:%S')}"
...
parser.add_argument('--dataset_path', type=str, default="data/PASCAL/pascal.csv", required=False)
parser.add_argument('--image_base_path', type=str, default="data/PASCAL/VOCdevkit/VOC2007/JPEGImages", required=False)
...
def method_wrapper(question, object_of_interest, image_path, working_dirname):
    ...
    object_aware_image_divider.detect_area(
        ...
        gdino_prompt=f"a group of {object_of_interest}s. {object_of_interest}s.",
    ...
    for area_image_path in sorted(
        ...
        object_aware_image_divider.divide_by_prompt(
            ...
            prompt="{}.".format(object_of_interest),
    ...
    for subimage_path in sorted(
        ...
        with open(subimage_path, "rb") as f:
            subimage = f.read()
        count = object_counter(question, object_of_interest, subimage)
...
def main():
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        logger.debug("category_name: %s", row["category_name"])
        logger.debug("image_url: %s", row["image_name"])
        logger.debug("gold answer: %s", row["answer"])
        image_path = f"{args.image_base_path}/{row['image_name']}"

        working_dir = f"{WORKING_DIRECTORY_BASE_NAME}/{row['image_name']}"
        os.mkdir(working_dir)
        logger.debug("output directory: %s", working_dir)
        question = f"How many {row['category_name']} are visible?"
        count = method_wrapper(
            question, row["category_name"], image_path, working_dir
        )
        out_df.append(
            row.tolist()
            + [

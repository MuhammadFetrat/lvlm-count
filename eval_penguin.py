...
logger.info("Super Resolution: %s", args.super_resolution)
...
def object_counter(question, object_of_interest_plural, image):
...
def method_wrapper(object_of_interest_singular_form, image_path, working_dirname):
    ...
    object_aware_image_divider.detect_area(
        ...
        gdino_prompt=f"a group of {object_of_interest_singular_form}s. {object_of_interest_singular_form}s.",
    ...
    for area_image_path in sorted(
        ...
        object_aware_image_divider.divide_by_prompt(
            ...
            prompt=f"{object_of_interest_singular_form}.",
    ...
    for subimage_path in sorted(
        ...
        with open(subimage_path, "rb") as f:
            subimage = f.read()
        question = f"How many {object_of_interest_singular_form}s are visible in the image?"
        count = object_counter(question, f"{object_of_interest_singular_form}s", subimage)
...
def main():
    ...
    for index, row in tqdm(df.iterrows(), total=len(df)):
        object_of_interest_singular_form = "penguin"
        logger.debug("image_url: %s", row["image_name"])
        logger.debug("gold answer max: %s", row["max"])
        logger.debug("gold answer median: %s", row["median"])
        image_path = f"{args.image_base_path}/{row['image_name']}.JPG"

        working_dir = f"{WORKING_DIRECTORY_BASE_NAME}/{index}.{row['image_name']}"
        os.mkdir(working_dir)
        logger.debug("output directory: %s", working_dir)
        count = method_wrapper(object_of_interest_singular_form, image_path, working_dir)
        out_df.append(
            row.tolist()
            + [
                count,
